...
// Orientações finais: a face sorteada termina 100% de frente.
 // Durante o giro o cubo permanece tridimensional; ao parar, a leitura é inequívoca.
...
async function animateDice(roll) {
  ...
  resultDesc.textContent = "A face ecológica está sendo sorteada.";
  subtitle.textContent = "Acompanhe o cubo até a face parar de frente.";
  ...
  resultValue.innerHTML =
    `Casa <strong>${roll}</strong> <span class="eco-result-icon">${theme.icon}</span>`;
  resultDesc.textContent = theme.label;
  subtitle.textContent =
    "Confira a face sorteada e, quando estiver pronto, siga para a pergunta.";
...
function getDiceAdvanceSteps(roll) {
  // Regra do jogo:
  // a casa onde o jogador já está é a 1ª casa da contagem.
  // Ex.: estando na casa 8 e tirando 4:
  // 8 (1), 9 (2), 10 (3), 11 (4) → destino casa 11.
  return Math.max(0, Number(roll) - 1);
}
...
async function rollDice() {
  ...
  const steps = getDiceAdvanceSteps(roll);

  state.pendingDice = roll;
  state.pendingSteps = steps;

  // A casa atual conta como a primeira casa.
  const targetCell = Math.min(TOTAL_CELLS, player.position + steps);
  ...
  setTurnMessage(`Casa ${roll}!`, "A casa atual é a primeira da contagem. Responda corretamente para concluir a jogada.");
    ...
    : (
        correct
          ? (state.pendingSteps > 0
              ? `Contar até a casa ${state.pendingDice}`
              : "Concluir a jogada")
          : "Passar a vez"
      );
}
...
async function continueAfterQuestion() {
  ...
  if (state.pendingSteps === 0) {
    setTurnMessage("🧭 Contagem concluída", `${player.name} contou a própria casa como a casa 1 e permanece na casa ${player.position}.`);
    await delay(500);
    $("continue-btn").disabled = false;
    advanceTurn();
    return;
  ...
  setTurnMessage(
    "🧭 Contando as casas!",
    `${player.name} conta a casa atual como 1 e segue até a ${state.pendingDice}ª casa da contagem.`
  );
  await movePlayerBy(player, state.pendingSteps);
