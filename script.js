...
function setDiceOverlay(open) {
  ...
  if (!open) overlay.classList.remove("result-ready");
}
...
async function animateDice(roll) {
  ...
  if (!cube || !resultValue || !resultDesc) {
    // Fallback: se algo impedir a renderização do dado, a partida continua.
  ...
  setDiceOverlay(true);
  resultValue.textContent = "⏳ Rolando...";
  resultDesc.textContent = "A natureza está definindo seu movimento.";

  // Cada jogada acumula várias voltas completas e termina na orientação exata
  // da face sorteada. Como a base sempre é múltipla de 360°, nenhuma face some.
  ...
  // Pequeno atraso para garantir que o navegador pinte o overlay antes do giro.
  await delay(90);
...
async function rollDice() {
  ...
  setTurnMessage("🎲 Dado em movimento...", "Vamos descobrir quantas casas estão em jogo.");
...
function nextQuestion() {
  while (state.questionQueue.length) {
    ...
    // Dupla garantia: mesmo que a fila seja alterada futuramente,
    // uma questão já utilizada nesta partida nunca será exibida de novo.
